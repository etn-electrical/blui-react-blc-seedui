    ...
    return (
        <Box sx={RegCompleteContainerStyles(theme)}>
            <Box sx={{ display: 'flex', justifyContent: 'center' }}>
                <AccountCircle color={'primary'} sx={{ fontSize: 54, height: '64px', width: '64px' }} />
            </Box>
            <Typography sx={{ mt: '57px', mb: '35px' }} variant="h6" color="inherit">
                Welcome {responseData.name}!
            </Typography>
            ...
            <List sx={{ listStyleType: 'disc', pl: 5 }}>
                {Object.keys(roleSiteList).map((key, index) => (
                    <Typography variant="subtitle2" color={'textSecondary'} key={index}>
                        <ListItem sx={{ display: 'list-item', padding: '4px 4px' }}>
                            {' '}
                            {key}
                            {roleSiteList[key].length ? ':' : ''} {roleSiteList[key].join(', ')}
                        </ListItem>
                    </Typography>
                ))}
            </List>
            ...
            <Typography sx={{ mt: '20px' }} variant="subtitle2" color={'textSecondary'}>
                You can now Login using the email address and password you specified during the account registration
                process
            </Typography>
        </Box>
    );
